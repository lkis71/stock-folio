...
### v2.0 (2024-12-24)
- 캘린더/리스트 탭 위치 일관성 개선 (상단 여백 통일)
- 빈 데이터 상태 개선 (통계 섹션 표시, 매매 기록 섹션 내부 메시지)
- 삭제 방식 변경 (스와이프 제거, 상세 화면 삭제 버튼 추가)
- 삭제 확인 Alert 구현

### v1.0 (2024-12-24)
- 초기 작성
- 매매 일지 목록, 작성/수정 화면 설계
- 통계 섹션 UI 정의 (승률 제거)
- 필터링 기능 화면 설계
...
### 1.2 설계 범위
- 매매 일지 목록 화면 (필터링 기능, 상단 여백 일관성, 빈 데이터 상태)
- 매매 일지 작성/수정 화면 (저장 버튼 개선, 삭제 버튼 추가)
- 통계 섹션 (승률 제거, 빈 상태 표시)
- 삭제 기능 (스와이프 제거, 상세 화면 버튼 추가, 확인 Alert)

### 1.3 최근 변경사항 (v2.0)

#### 캘린더/리스트 탭 위치 일관성
- 리스트 뷰의 상단 여백을 캘린더 뷰와 동일한 위치에서 시작하도록 수정
- 통계 섹션의 패딩 값 일관화

#### 빈 데이터 상태 개선
- 데이터가 없을 때도 통계 섹션 표시 (값은 0원/0건)
- 매매 기록 섹션 헤더 유지
- "등록된 매매 기록이 없습니다" 메시지를 매매 기록 섹션 내부에 표시
- 기존 "매매 일지가 없습니다" 단순 메시지 제거

#### 삭제 방식 변경
- 리스트의 스와이프 삭제 기능 제거
- 상세 페이지(AddTradingJournalView)에 삭제 버튼 추가 (수정 모드에서만 표시)
- 삭제 확인 Alert 구현
  - 제목: "삭제 확인"
  - 메시지: "이 매매 기록을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다."
  - 취소/삭제 버튼 제공
...
**인터랙션**:
- **Tap**: 수정 화면 표시 및 `AddTradingJournalView` 로드
...
##### 저장 및 삭제 버튼 영역
**변경사항**: 포커스와 무관하게 항상 표시, 삭제 버튼 추가 (수정 모드)
...
.alert("삭제 확인", isPresented: $showDeleteConfirmation) {
...
### Phase 1 (필수)
1. 승률 통계 제거
2. 매매 기록 탭 → 수정 화면 (기존 유지)
3. 저장 버튼 항상 표시 (기존 유지)
4. 캘린더/리스트 탭 상단 여백 일관성 유지
5. 빈 데이터 상태: 통계 섹션 + "등록된 매매 기록이 없습니다" 메시지 표시
6. 스와이프 삭제 기능 제거 (리스트 뷰)
7. 수정 모드에 삭제 버튼 추가 (AddTradingJournalView)
8. 삭제 확인 Alert 추가

### Phase 2
9. 필터 UI 구현 (일별/월별/년도별)
10. 필터링 로직 연동
...
- [ ] 통계 섹션에 승률 미표시
- [ ] 데이터 없을 때 통계 섹션과 "등록된 매매 기록이 없습니다" 메시지 표시
- [ ] 캘린더 탭과 리스트 탭의 상단 여백이 일관됨
- [ ] 수정 모드에서 삭제 버튼 표시 및 작동
- [ ] 삭제 버튼 탭 시 확인 Alert 표시
- [ ] Alert에서 취소 버튼 탭 시 작업 취소
- [ ] Alert에서 삭제 버튼 탭 시 기록 삭제 및 목록 갱신
