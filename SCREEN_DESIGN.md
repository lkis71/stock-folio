...
**탭 아이템:**
- **포트폴리오**: SF Symbol "chart.pie.fill"
- **매매 일지**: SF Symbol "book.fill"
...
│                                     │
│  ┌─ 필터 ────────────────────────┐  │
│  │ 📅 전체 기간  │ 📈 전체  │ 🏷️ 전체│  │  ← 필터 바 (날짜/유형/종목)
│  └────────────────────────────────┘  │
│                                     │
│  ┌─────────────────────────────────┐ │
│  │ 🟢 매수      2024.12.18         │ │  ← 매매 일지 항목
│  │ 삼성전자                         │ │
│  │ 10주 × ₩70,000                  │ │
│  │ ₩700,000                        │ │
│  └─────────────────────────────────┘ │
...
```
┌─────────────────────────────────────┐
│ 매매 통계                            │  ← Navigation Bar
├─────────────────────────────────────┤
│                                     │
│  ┌───────────────┬───────────────┐  │
│  │ 총 매매 횟수   │ 실현 손익      │  │  ← 2단 요약 카드
│  │ 25건          │ +₩500,000     │  │
│  │ 매수 15 매도 10│ +12.5%        │  │
│  └───────────────┴───────────────┘  │
│                                     │
...
#### 종목별 통계
- **정렬**: 거래 횟수 내림차순
- **표시**: 순위, 종목명, 거래 횟수, 총 손익

### 13.5 빈 상태 (Empty State)
...
#### 매매 유형 색상
- **매수**: `Color.green` (배경: opacity 0.1)
- **매도**: `Color.red` (배경: opacity 0.1)
...
#### 리스트 업데이트
- **추가**: Fade + Slide from top (0.3초)
- **삭제**: Fade + Slide to trailing (0.3초)
- **필터 적용**: Crossfade (0.2초)

#### 통계 차트
- **로딩**: 왼쪽에서 오른쪽으로 애니메이션 (0.5초)
- **업데이트**: Smooth transition (0.3초)
